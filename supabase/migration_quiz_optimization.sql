-- ╔═══════════════════════════════════════════════════╗
-- ║  Migration: Quiz optimization                    ║
-- ║  Submit final score once at end (like Sequence)  ║
-- ║  Uses creator_score/opponent_score + time cols    ║
-- ╚═══════════════════════════════════════════════════╝


-- ╔═══════════════════════════════════════════════════╗
-- ║  1. submit_quiz_result — single submission at end ║
-- ╚═══════════════════════════════════════════════════╝

CREATE OR REPLACE FUNCTION submit_quiz_result(
  p_duel_id  UUID,
  p_user_id  UUID,
  p_score    INTEGER,   -- number of correct answers (0-5)
  p_time     REAL       -- total time spent (seconds)
)
RETURNS JSONB
...
BEGIN
  ...
  IF v_duel.game_type != 'quiz' THEN
    RETURN jsonb_build_object('status', 'error', 'error', 'not_quiz');
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  PERFORM admin_log('error', 'rpc:submit_quiz_result', SQLERRM,
...
-- ╔═══════════════════════════════════════════════════╗
-- ║  2. Update finalize_duel — quiz tiebreak via time ║
-- ║     Use creator_time/opponent_time instead of     ║
-- ║     querying duel_answers                         ║
-- ╚═══════════════════════════════════════════════════╝
...
BEGIN
  ...
  ELSIF d.opponent_score > d.creator_score THEN
    ...
    v_loser  := d.creator_id;
  ELSE
    -- Тай-брейк: все игры используют creator_time/opponent_time

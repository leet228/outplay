-- ╔═══════════════════════════════════════════════════╗
-- ║  Migration: Gradient game support                ║
-- ║  Score = total color distance (lower = better)   ║
-- ╚═══════════════════════════════════════════════════╝


-- ╔═══════════════════════════════════════════════════╗
-- ║  0. Extend CHECK constraints for 'gradient'      ║
-- ╚═══════════════════════════════════════════════════╝
...
ALTER TABLE matchmaking_queue ADD CONSTRAINT matchmaking_queue_category_check
  CHECK (category IN ('general','history','science','sport','movies','music','quiz','blackjack','sequence','reaction','hearing','gradient'));
...
ALTER TABLE game_invites ADD CONSTRAINT game_invites_game_type_check
  CHECK (game_type IN ('quiz', 'blackjack', 'sequence', 'reaction', 'hearing', 'gradient'));


-- ╔═══════════════════════════════════════════════════╗
-- ║  1. submit_gradient_result                       ║
-- ╚═══════════════════════════════════════════════════╝

CREATE OR REPLACE FUNCTION submit_gradient_result(
  p_duel_id  UUID,
  p_user_id  UUID,
  p_score    INTEGER,   -- total color distance (lower = better)
  p_time     REAL       -- total time spent
)
RETURNS JSONB
...
BEGIN
  ...
  IF v_duel.game_type != 'gradient' THEN
    RETURN jsonb_build_object('status', 'error', 'error', 'not_gradient');
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  PERFORM admin_log('error', 'rpc:submit_gradient_result', SQLERRM,
...
-- ╔═══════════════════════════════════════════════════╗
-- ║  2. Update finalize_duel — gradient support      ║
-- ║     Gradient: lower score (color dist) wins      ║
-- ╚═══════════════════════════════════════════════════╝
...
BEGIN
  ...
  ELSIF d.game_type = 'blackjack' THEN
    IF d.creator_score > 21 AND d.opponent_score > 21 THEN
      ...
      ELSE
        IF random() < 0.5 THEN v_winner := d.creator_id; v_loser := d.opponent_id;
  ...
  ELSIF d.game_type IN ('hearing', 'gradient') THEN
    -- Слух и Градиент: побеждает меньший score (разница Hz / цвета)
  ...
  ELSE
    ...
    v_creator_time := COALESCE(d.creator_time, 75);
    v_opp_time := COALESCE(d.opponent_time, 75);
    IF v_creator_time <= v_opp_time THEN v_winner := d.creator_id; v_loser := d.opponent_id;
    ELSE v_winner := d.opponent_id; v_loser := d.creator_id; END IF;
  END IF;

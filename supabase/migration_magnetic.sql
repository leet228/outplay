...
-- =============================================
--
-- The slot's full event lifecycle (base spin + optional bonus FS
-- run + buy-bonus + reconciliation) is wrapped in ONE round on
-- the server:
--
--   start_magnetic_round  — atomic stake debit, opens a round.
--                           For buy-bonus, debits stake × 100
--                           and tags the round is_bought=true.
...
-- user closes the tab mid-bonus. The Spin button stays disabled
-- until finish returns.
--
-- RTP target: 95.5 % verified by scripts/magnetic-rtp-sim.js on
--             1 M-spin Monte-Carlo (base ~54 %, bonus ~42 %).

-- ── 1. Seed slot_stats row for magnetic if missing ────────────────
INSERT INTO slot_stats (slot_id, target_rtp, max_house_deficit_rub)
  VALUES ('magnetic', 0.955, 10000)
  ON CONFLICT (slot_id) DO NOTHING;
UPDATE slot_stats SET target_rtp = 0.955 WHERE slot_id = 'magnetic';


-- ── 2. start_magnetic_round ──────────────────────────────────────
...
DROP FUNCTION IF EXISTS start_magnetic_round(UUID, INTEGER, BOOLEAN);

CREATE OR REPLACE FUNCTION start_magnetic_round(
  p_user_id      UUID,
  p_stake_rub    INTEGER,
...
BEGIN
  ...
  -- Auto-abort prior pending magnetic rounds for this user
  -- (crash recovery: tab closed mid-spin shouldn't lock the next
  -- start). The aborted round's stake is forfeited — this is
  -- the standard slot behaviour across the codebase.
  UPDATE slot_rounds
     SET outcome = 'aborted', finished_at = NOW()
   WHERE user_id = p_user_id AND outcome = 'pending' AND slot_id = 'magnetic';
  ...
  SELECT current_pnl_rub, max_house_deficit_rub, enabled
    ...
    FROM slot_stats WHERE slot_id = 'magnetic';

  IF v_house_pnl IS NULL THEN
    INSERT INTO slot_stats (slot_id, target_rtp, max_house_deficit_rub)
      VALUES ('magnetic', 0.955, 10000) ON CONFLICT DO NOTHING;
  ...
  VALUES (
    p_user_id, 'magnetic', p_stake_rub, v_bias,
    'open', NULL,
...
EXCEPTION WHEN OTHERS THEN
  PERFORM admin_log('error', 'rpc:start_magnetic_round', SQLERRM,
...
BEGIN
  ...
  -- of rounds until it recovers. Buy-bonus rounds get a floor of
  -- the debited amount instead, so the player at least breaks
  -- even on the cost of the buy.
  ...
  -- including buy-bonus surcharge) and effective payout — so RTP
  -- stays accurate across the mixed flow.
  INSERT INTO slot_stats (slot_id, total_games, total_wagered_rub, total_paid_rub, current_pnl_rub)
    VALUES ('magnetic', 1, v_debited, v_payout_to_pay, v_debited - v_payout_to_pay)
...
EXCEPTION WHEN OTHERS THEN
  PERFORM admin_log('error', 'rpc:finish_magnetic_round', SQLERRM,
...
-- ── 4. Live feed: recognise magnetic ─────────────────────────────
...
-- ── 5. Live feed: include magnetic in fake-event seeder ──────────
...
GRANT EXECUTE ON FUNCTION start_magnetic_round(UUID, INTEGER, BOOLEAN) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION finish_magnetic_round(UUID, INTEGER)         TO authenticated, anon;

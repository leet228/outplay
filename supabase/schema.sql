...
-- ╔═══════════════════════════════════════════════════════════╗
-- ║  Индекс + функция: таймаут заброшенных PvP-дуэлей       ║
-- ╚═══════════════════════════════════════════════════════════╝
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_duel.game_type IN ('sequence', 'reaction', 'hearing', 'gradient', 'race') THEN
    ...
    -- Оба играли — должно было финализироваться само
    IF v_creator_played AND v_opponent_played THEN
      v_count_skip := v_count_skip + 1;
      CONTINUE;
    END IF;

    -- Один играл, другой вышел → победа активному

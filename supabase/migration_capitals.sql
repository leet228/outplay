-- ╔═══════════════════════════════════════════════════╗
-- ║  Migration: Capitals game support                ║
-- ║  Score = total km error across rounds            ║
-- ║  (lower = better, like race/hearing/gradient)    ║
-- ╚═══════════════════════════════════════════════════╝
...
-- ║     Capitals: lower score (total km) wins        ║
-- ╚═══════════════════════════════════════════════════╝
...
BEGIN
  ...
  ELSIF d.game_type IN ('hearing', 'gradient', 'race', 'capitals') THEN
    -- Слух, Градиент, Гонка, Столицы: побеждает меньший score
    IF d.creator_score < d.opponent_score THEN v_winner := d.creator_id; v_loser := d.opponent_id;
    ELSIF d.opponent_score < d.creator_score THEN v_winner := d.opponent_id; v_loser := d.creator_id;
    ELSE IF random() < 0.5 THEN v_winner := d.creator_id; v_loser := d.opponent_id; ELSE v_winner := d.opponent_id; v_loser := d.creator_id; END IF; END IF;
...
BEGIN
  ...
  ELSIF v_inv.game_type = 'capitals' THEN v_category := 'capitals';

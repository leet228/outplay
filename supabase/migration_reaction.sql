-- ╔═══════════════════════════════════════════════════╗
-- ║  Migration: Reaction game support                ║
-- ║  Reuses creator_time / opponent_time columns     ║
-- ║  from migration_sequence.sql                     ║
-- ╚═══════════════════════════════════════════════════╝
...
CREATE OR REPLACE FUNCTION submit_reaction_result(
  p_duel_id  UUID,
  p_user_id  UUID,
  p_score    INTEGER,
  p_time     REAL
...
BEGIN
  ...
  IF v_duel.game_type != 'reaction' THEN
    RETURN jsonb_build_object('status', 'error', 'error', 'not_reaction');
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  PERFORM admin_log('error', 'rpc:submit_reaction_result', SQLERRM,
...
-- ╔═══════════════════════════════════════════════════╗
-- ║  2. Update finalize_duel — reaction tiebreak     ║
-- ║     Change 'sequence' → IN ('sequence','reaction')║
-- ╚═══════════════════════════════════════════════════╝
...
BEGIN
  ...
  ELSIF d.game_type = 'reaction' THEN
    -- Реакция: побеждает тот у кого меньше среднее время (creator_time/opponent_time)

...
// ── Live feed (slot wins/losses ribbon) ───────────────
// Single shared feed of slot outcomes — real ones from triggers on
// slot_rounds / rocket_bets, fake ones generated by a server-side
// pg_cron job every few seconds. Clients fetch the initial page
// once, then subscribe to INSERTs via Realtime — no polling.

export async function getLiveFeed(limit = 30) {
  const { data, error } = await supabase.rpc('get_live_feed', { p_limit: limit })
  if (error) {
    console.error('getLiveFeed error:', error)
    logClientError('live_feed.fetch', error.message, { code: error.code, limit })
    return []
  }
  return data ?? []
}
